...
def load_house_attributes(inputPath):
    ...
    for (zipcode, count) in zip(zipcodes, counts):
        # the zip code counts for our housing dataset is *extremely* unbalanced (some only having 1 or 2 houses per zip code) so let's sanitize our data by
...
def process_house_attributes(df, train, test):
    ...
    # one-hot encode the zip code categorical data (by definition of one-hot encoding, all output features are now in the range [0, 1])
...
def load_house_images(df, inputPath):
    ...
    for i in df.index.values:
        ...
        # initialize our list of input images along with the output image after *combining* the four input images
        inputImages = []
        ...
        # tile the four input images in the output image such the first image goes in the top-right corner, the second image in the top-left corner,
